...
def parse(input_directory, output_file, mapping_semantics=False):
    ...
    for split_by_date in os.listdir(input_directory):
        data_split_by_date = os.path.join(input_directory, split_by_date);
        if os.path.isfile(data_split_by_date):
            print "skip file %s" % data_split_by_date;
            continue;
        
        for split_by_index in os.listdir(data_split_by_date):
            data_split_by_date_index = os.path.join(data_split_by_date, split_by_index);
